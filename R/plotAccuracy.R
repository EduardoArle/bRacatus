...
plotAccuracy <- function(acc,regional=TRUE,reg.by="country",borders=TRUE,col.features="khaki",col.bg="azure2",plot.range=FALSE,range=NULL,box=FALSE){
  ...
  plot(map,col=col.features,bg=col.bg,border=bord,main=unique(acc$species),font.main=3)
  
  if(plot.range){plot(range,add=TRUE,border=NA,col="#1c9099")}
  ...
  if(box){
    ...
    gradientLegend(valRange=c(0,1),pos=c(1.03,0.6,1.045,0.89),side=4,color=colacc(20),n.seg=3)
  }else{
