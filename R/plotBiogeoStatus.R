...
#' @param plot.range logical, if TRUE, range maps should be provided as a shapefile in argument range.
...
plotBiogeoStatus <- function(biogeo,regional=TRUE,reg.by="country",borders=TRUE,col.features="khaki",col.bg="azure2",plot.range=FALSE,range=NULL,box=FALSE){
  ...
  colbiogeo <- colorRampPalette(c("#b2182b","#ef8a62","#fddbc7","#fddbc7","#f7f7f7","#f7f7f7","#f7f7f7","#d1e5f0","#d1e5f0","#67a9cf","#2166ac"))
  col_pts_biogeo <- colbiogeo(100)[cut(c(0,1,biogeo$biogeoStatus),breaks = 100)]
  col_pts_biogeo <- col_pts_biogeo[-c(1,2)]
  
  if(borders){bord="black"}else{bord=NA}
  
  par(mfrow=c(1,1),mar=c(1.5,1.5,1.5,5))
  plot(map,col=col.features,bg=col.bg,border=bord,main=unique(biogeo$species),font.main=3)
  ...
  if(box){
    plot(CP,add=TRUE)
    points((extent(CP)[1]+extent(CP)[2])/2,extent(CP)[3],pch=3,cex=.8)
    text((extent(CP)[1]+extent(CP)[2])/2,extent(CP)[3]-(extent(CP)[4]-extent(CP)[3])/22,round((extent(CP)[1]+extent(CP)[2])/2,2),cex=.8)
    points(extent(CP)[2],(extent(CP)[3]+extent(CP)[4])/2,pch="_")
    text(extent(CP)[2]+(extent(CP)[2]-extent(CP)[1])/40,(extent(CP)[3]+extent(CP)[4])/2,round((extent(CP)[3]+extent(CP)[4])/2,2),cex=.8,srt=90)
    points(extent(CP)[2]-(extent(CP)[2]-extent(CP)[1])/20,extent(CP)[3]+(extent(CP)[4]-extent(CP)[3])/20,pch=24,bg="gray30",cex=1.5)
    text(extent(CP)[2]-(extent(CP)[2]-extent(CP)[1])/20,extent(CP)[3]+(extent(CP)[4]-extent(CP)[3])/9,"N",cex=.6)
    gradientLegend(valRange=c(0,1),pos=c(1.03,0.6,1.045,0.89),side=4,color=colbiogeo(20),n.seg=3)
  }else{
